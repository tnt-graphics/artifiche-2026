...
function get_shop_filter_query() {
	if ( ! is_admin() && is_post_type_archive( 'product' ) ) {
		...
		$taxonomies = array();
		$metaquery  = array();
		// array_push( $query, array( 'orderby' => $orderby ) );
		// array_push( $query, array( 'meta_key' => 'neu_flag' ) );
		...
		// $metaquery
		// );
		if ( ( isset( $_GET['name'] ) || isset( $_GET['kategorie'] ) || isset( $_GET['kuenstler'] ) || isset( $_GET['land'] ) || isset( $_GET['stilrichtung'] ) || isset( $_GET['search'] ) || isset( $_GET['sold_posters'] ) ) && ! isset( $_GET['sortby'] ) ) {
			$query_string      = array();
