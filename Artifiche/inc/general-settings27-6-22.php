...
//		$artf_nonce = $_REQUEST['artf_filter'];
		...
		if ( isset( $_SERVER['QUERY_STRING'] ) || isset( $_COOKIE['artf_sale_flag'] ) ) {
			...
			if ( isset( $_GET['sold_posters'] ) ) {
				$sold_posters = $_GET['sold_posters'];
			} else {
				$sold_posters = 0;
			}
...
if ( defined( 'YITH_WCWL' ) && ! function_exists( 'yith_wcwl_get_items_count' ) ) {
  ...
  if ( defined( 'WPFDA_PLUGIN_FILE' ) ) {
	...
	//if ( current_user_can( 'administrator' ) ) {	
		$html .= esc_html__( 'If you delete your account, all data will be permanently erased.
		You can create a new account at any time.', 'wp-frontend-delete-account' );
