...
/*
Element Description: Artifiche Testimonial Small
*/
...
vc_map(
	array(
		...
		'base'        => 'artifiche_testimonial_small',
		'icon'        => get_template_directory_uri() . '/img/logo2.svg',
		'category'    => __( 'Artifiche Elements', 'artifiche' ),
		'description' => __( 'Artifiche Home-Poster (links klein)', 'artifiche' ),
		'params'      => array_merge(
			array(
				array(
					'type'        => 'autocomplete',
...
add_shortcode( 'artifiche_testimonial_small', 'artifiche_testimonial_small_fun' );


// Element HTML
function artifiche_testimonial_small_fun( $atts, $content = null ) {
	global $post;
	...
	<a href="'. get_permalink( $post->ID ) .'"><img  src="'. site_url() .'/artifiche-images/posters_extralarge/'. $poster_id .'.jpg" alt="'. $alt_text .'" /></a>
...
add_filter( 'vc_autocomplete_artifiche_testimonial_small_post1_callback', 'artifiche_testimonial_small_search', 10, 1 );
add_filter( 'vc_autocomplete_artifiche_testimonial_small_post1_render', 'vc_include_field_render', 10, 1 );

function artifiche_testimonial_small_search( $search_string ) {
	$query = $search_string;
	$data  = array();
